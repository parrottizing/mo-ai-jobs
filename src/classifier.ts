...
function buildPrompt(job: ClassifierJob, promptInput: PromptInput): string {
  return [
    ...
    "",
    "CANDIDATE PROFILE:",
    "- Total professional experience: ~3.5 years.",
    ...
    "- No proven professional track record in: product management, content strategy/marketing/editorial, people leadership, management consulting/customer success, product design leadership, applied ML research, or advanced data science.",
    ...
    "1) Read mandatory sections first: 'Required', 'Minimum', 'Basic Qualifications', 'Must have', and title seniority.",
    "2) Ignore preferred/nice-to-have sections for eligibility unless they are explicitly mandatory.",
    "3) If any mandatory item is missing from the profile, return NO.",
    "4) If evidence is ambiguous, return NO.",
    "",
    "HARD-FAIL RULES (any one => NO):",
    "1) Mandatory minimum experience exceeds 3.5 years, or requires years in a function not in profile.",
    ...
    "3) Functional background mismatch: role requires proven PM, content strategy/marketing, consulting/customer success, design leadership, ML research, or deep data-science background.",
    ...
    "6) Posting is expired or closed.",
    "",
    "YES RULE:",
    "- Return YES only when every mandatory requirement is satisfied and no hard-fail rule is triggered.",
    "- AI-native language alone is never enough for YES.",
    "",
    "OUTPUT FORMAT (exactly 3 lines):",
    "Line 1: YES or NO",
    "Line 2: REASON_CODES: <comma-separated from: GOOD_AI_NATIVE_FIT, MISSING_REQUIRED_EXPERIENCE, SENIORITY_MISMATCH, FUNCTIONAL_BACKGROUND_MISMATCH, MISSING_REQUIRED_CREDENTIAL, ADVANCED_DEGREE_REQUIRED, MANUAL_CODING_REQUIRED, DOMAIN_MISMATCH, EXPIRED_OR_CLOSED, INSUFFICIENT_EVIDENCE>",
